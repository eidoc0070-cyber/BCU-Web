...
import {
  AddPartCommand,
  DeletePartCommand,
} from './commands/hierarchy-commands';
...
export class BCUController {
  ...
  private initEventSubscriptions() {
            ...
            if (part) {
              ...
              if (oldValue !== data.value) {
                ...
                if (data.field === AnimProp.Parent && data.value !== -1) {
                  if (
                    this.bridge!.isAncestor &&
                    this.bridge!.isAncestor(partIdx, data.value)
                  ) {
                    this.ui?.showToast(
                      'Hierarchy cycle detected! Action blocked.',
                      'error',
                    );
                    return;
                  }
    ...
    eventBus.on('TRANSFORM_COMMITTED', (data) => {
      if (this.bridge && this.state.getStatus().isReady) {
        ...
        const byField: Map<
          number,
          { idxs: number[]; olds: Map<number, number>; val: number }
        > = new Map();
    ...
    eventBus.on('FRAME_SEEK', (data) => {
      if (
        this.bridge &&
        !this.state.getStatus().isPlaying &&
        this.state.getStatus().isReady
      ) {
    ...
    eventBus.on('KEYFRAME_MODIFIED', (data) => {
      if (this.bridge && this.state.getStatus().isReady) {
        ...
        if (state && state.animation && state.animation.anim) {
          const part = state.animation.anim.parts.find(
            (p: any) =>
    ...
    eventBus.on('KEYFRAME_ADDED', (data) => {
      if (this.bridge && this.state.getStatus().isReady) {
        ...
        this.history.execute(cmd);
        this.log(
          `Added keyframe: Part ${data.partIdx}, Type ${data.modifType}, Frame ${data.frame}`,
        );
        triggerSave();
      }
    });
    ...
    eventBus.on('PART_DELETED', (data) => {
      if (this.bridge && this.state.getStatus().isReady) {
        if (confirm(`Are you sure you want to delete part ${data.partIdx}?`)) {
          const cmd = new DeletePartCommand(
            this.bridge,
            this.state,
            data.partIdx,
          );
  ...
  private applySessionData(session: EditorSession) {
    ...
    try {
      ...
      this.log(
        `Session restored: ${session.animId} at frame ${session.currentFrame}`,
      );
    } catch (e) {
  ...
  public resize() {
    ...
    this.gizmoCanvas.width = w;
    this.gizmoCanvas.height = h;
    this.imgcutCanvas.width = w;
    this.imgcutCanvas.height = h;
  ...
  private drawPlaceholder() {
    ...
    const msg = !this.state.getStatus().isReady
      ? 'Preparing Engine...'
  ...
  public async recreateEngine() {
    ...
    if (!isTestEnv && typeof BCUEngine.init === 'function') {
      ...
      this.log(
        'Running in test/mock environment. Skipping real WASM reinitialization.',
      );
    }
  }
}
