import { ANIM_PROP_NAMES, AnimProp, InterpolationType } from '../constants';
  ...
  public update(
    ...
    if (selectedParts.length === 0 && kfSelection.length === 0) {
      this.container.innerHTML =
        '<div style="color: var(--text-secondary); font-size: 0.7rem; text-align: center; margin-top: 2rem;">No selection</div>';
      return;
    }
    ...
    const getInterpolatedValue = (p: any, field: AnimProp) => {
      ...
      switch (field) {
        case AnimProp.PosX:
          return (
            (p.prev_state.pos?.x ?? raw) * (1 - alpha) +
            (p.curr_state.pos?.x ?? raw) * alpha
          );
        case AnimProp.PosY:
          return (
            (p.prev_state.pos?.y ?? raw) * (1 - alpha) +
            (p.curr_state.pos?.y ?? raw) * alpha
          );
        case AnimProp.ScaleX:
          return (
            (p.prev_state.sca?.x ?? raw) * (1 - alpha) +
            (p.curr_state.sca?.y ?? raw) * alpha
          );
        case AnimProp.ScaleY:
          return (
            (p.prev_state.sca?.y ?? raw) * (1 - alpha) +
            (p.curr_state.sca?.y ?? raw) * alpha
          );
        case AnimProp.Rotation:
          return (
            (p.prev_state.angle ?? raw) * (1 - alpha) +
            (p.curr_state.angle ?? raw) * alpha
          );
        case AnimProp.Opacity:
          return (
            (p.prev_state.opacity ?? raw) * (1 - alpha) +
            (p.curr_state.opacity ?? raw) * alpha
          );
        default:
          return raw;
      }
    ...
    const getMixedValue = (field: AnimProp) => {
      ...
      const isMixed = selectedParts.some(
        (p) => Math.abs(getInterpolatedValue(p, field) - firstVal) > 0.001,
      );
    ...
    const renderPropRow = (
      ...
      const isPlaying = this.stateManager.getStatus().isPlaying;
      const displayValue =
        typeof value === 'number' && isPlaying && animatable
          ? value.toFixed(1)
          : value;
    ...
    if (kfSelection.length > 0) {
      ...
      if (kfData.length > 0) {
        ...
        const isMixedInterp = kfData.some(
          (d) => d!.interp !== kfData[0]!.interp,
        );
        const isMixedEasing = kfData.some(
          (d) => d!.easing !== kfData[0]!.easing,
        );
        const isMixedType = kfData.some((d) => d!.mType !== kfData[0]!.mType);

        const modifName = isMixedType
          ? 'Mixed Types'
          : ANIM_PROP_NAMES[kfData[0]!.mType as AnimProp] || 'Misc';
    ...
    this.container.querySelectorAll('.prop-input').forEach((input) => {
      input.addEventListener('change', (e) => {
        ...
        if (field === AnimProp.Parent && !isMultiPart) {
          if (
            PropertyValidator.wouldCreateCycle(
              primaryPart.index,
              value,
              allParts,
            )
        ...
        if (result.corrected) {
          value = result.value;
          eventBus.emit('SHOW_TOAST', {
            message: result.message || 'Value clamped to valid range',
            type: 'warning',
          });
        }

        eventBus.emit('PROPERTY_CHANGED', {
          partIdxs: this.currentPartIdxs,
          field,
          value,
          source: 'Inspector',
        });
    ...
    this.container.querySelectorAll('.kf-input').forEach((input) => {
      input.addEventListener('change', (e) => {
        ...
        const frameInput = this.container!.querySelector(
          '.kf-input[data-type="frame"]',
        ) as HTMLInputElement;
        const valueInput = this.container!.querySelector(
          '.kf-input[data-type="value"]',
        ) as HTMLInputElement;
        const interpSelect = this.container!.querySelector(
          '.kf-input[data-type="interp"]',
        ) as HTMLSelectElement;
        const easingInput = this.container!.querySelector(
          '.kf-input[data-type="easing"]',
        ) as HTMLInputElement;
        const easingSlider = this.container!.querySelector(
          '.kf-input[data-type="easing-slider"]',
        ) as HTMLInputElement;

        const frame = parseInt(frameInput.value, 10);
        const value = parseInt(valueInput.value, 10);
        const interp = parseInt(interpSelect.value, 10);
        let easing = easingInput ? parseInt(easingInput.value, 10) : 0;

        if (inputType === 'easing-slider' && easingSlider) {
          easing = parseInt(easingSlider.value, 10);
        }
        ...
        kfSelection.forEach((id) => {
          ...
          if (part) {
            ...
            if (move) {
              ...
              const validatedInterp = PropertyValidator.validateInterpolation(
                mType,
                interp,
              );
              ...
              changes.push({
                ...
                newData: {
                  frame: Number.isNaN(frame) ? fr : frame,
                  value: Number.isNaN(value) ? move[1] : value,
                  interp:
                    Number.isNaN(interp) || interp === -1
                      ? move[2]
                      : validatedInterp,
                  easing: Number.isNaN(easing) ? move[3] : easing,
                },
              });
            }
          }
        ...
        if (changes.length > 0) {
          ...
          if (
            inputType === 'interp' ||
            inputType === 'easing-slider' ||
            inputType === 'easing'
          ) {
            // Re-render UI to update dynamic interpolation fields or slider labels
            setTimeout(
              () =>
                this.update(selectedParts, anim, currentFrame, allParts, alpha),
              10,
            );
          }
        }
