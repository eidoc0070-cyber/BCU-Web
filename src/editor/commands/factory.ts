      ...
      switch (data.type) {
        case 'UPDATE_PROPERTY': {
          ...
          if (data.oldValues && typeof data.oldValues === 'object') {
            Object.entries(data.oldValues).forEach(([k, v]) =>
              oldValues.set(Number(k), Number(v)),
            );
          }
        ...
        case 'MODIFY_KEYFRAME':
          return new ModifyKeyframeCommand(
            bridge,
            data.partIdx,
            data.modifType,
            data.oldData,
        ...
        case 'BATCH': {
          const subCommands = data.commands
            .map((c: any) =>
              CommandFactory.deserialize(c, bridge, stateManager),
            )
            .filter((c: any) => c !== null);
        ...
        default:
          console.warn(
            `Unknown command type for deserialization: ${data.type}`,
          );
          return null;
      }
    } catch (e) {
