...
async function run() {
  ...
  const gizmoCanvas = document.getElementById(
    'gizmo-canvas',
  ) as HTMLCanvasElement;
  const imgcutCanvas = document.getElementById(
    'imgcut-canvas',
  ) as HTMLCanvasElement;
  ...
  try {
    ...
    const controller = new BCUController(
      engine,
      canvas,
      gizmoCanvas,
      imgcutCanvas,
    ...
    (window as any).debug = {
      ...
      forceRender: () => {
        if (controller.bridge) {
          ...
          if (status.animId === 'none') {
            console.warn(
              "[Debug] No active animation to render. Defaulting to 'walk' or first loaded.",
            );
      ...
      runDiagnostics: async () => {
        const { runDiagnostics, formatDiagnosticReport } = await import(
          './diagnostics'
        );
    ...
    // Initial load
    (window as any).loadCharacter = (url: string) =>
      controller.loadCharacter(url);
    controller.loadCharacter('assets/sample_unit');
  } catch (e) {
