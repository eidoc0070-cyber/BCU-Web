...
export async function runDiagnostics(): Promise<DiagnosticResult> {
    ...
    if (result.webGpuSupported) {
        try {
            ...
            if (adapter) {
                ...
                result.webGpuAdapter = info.description || adapter.name || 'Generic WebGPU Adapter';
            } else {
                result.errors.push('WebGPU navigator.gpu.requestAdapter() returned null.');
            }
        } catch (e: any) {
    ...
    if (gl) {
        ...
        if (debugInfo) {
            result.webGl2Renderer = gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL);
        } else {
    ...
    if (!result.secureContext) {
        result.errors.push('NOT IN SECURE CONTEXT: WebGPU requires HTTPS or localhost. If you are using an IP address, WebGPU will be disabled.');
    }
