  ...
  test('DeletePartCommand should capture state and restore correctly via Undo', () => {
        ...
        parts: [
          {
            index: 0,
            name: 'Root',
            raw_args: [-1, 0, 0, 0, 0, 0, 0, 0, 1000, 1000, 0, 1000, 0, 0],
          },
          {
            index: 1,
            name: 'To Delete',
            raw_args: [0, 0, 0, 0, 10, 20, 0, 0, 1000, 1000, 0, 1000, 0, 0],
          },
          {
            index: 2,
            name: 'Last',
    ...
    const bridge = {
      ...
      deletePart: (idx: number) => {
        ...
        mockState.animation.anim.parts = mockState.animation.anim.parts.filter(
          (p) => p.ints[0] !== idx,
        );
      ...
      restorePart: (idx: number, data: any, name: string, kfs: any[]) => {
        ...
        mockState.animation.parts.splice(idx, 0, {
          index: idx,
          name,
          raw_args: data,
        });
