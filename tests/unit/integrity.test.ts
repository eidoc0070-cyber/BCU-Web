...
describe('PropertyValidator Guard Rails Tests', () => {
  ...
  test('should clamp Rotation', () => {
    expect(PropertyValidator.clamp(AnimProp.Rotation, -40000).value).toBe(
      -36000,
    );
  ...
  test('should force Step interpolation for Parent, UnitID, Sprite, and Flips', () => {
    // Step only fields
    expect(
      PropertyValidator.validateInterpolation(
        AnimProp.Parent,
        InterpolationType.Linear,
      ),
    ).toBe(InterpolationType.Step);
    expect(
      PropertyValidator.validateInterpolation(
        AnimProp.Image,
    ...
    ).toBe(InterpolationType.Linear);
    expect(
      PropertyValidator.validateInterpolation(
        AnimProp.Opacity,
        InterpolationType.Easing,
      ),
    ).toBe(InterpolationType.Easing);
  });
