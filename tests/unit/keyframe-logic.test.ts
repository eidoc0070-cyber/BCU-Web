...
} from '../../src/editor/commands/animation-commands';
...
describe('Identity-based Keyframe Logic Tests', () => {
  ...
  test('ModifyKeyframeCommand should find keyframe by frame ID, not index', () => {
    ...
    mockState.animation.anim.parts[0].moves.unshift([
      -5,
      -100,
  ...
  test('DeleteKeyframeCommand should find keyframe by frame ID', () => {
    ...
    cmd.execute();
    expect(
      mockState.animation.anim.parts[0].moves.some((m: any) => m[0] === 10),
    ).toBe(false);
