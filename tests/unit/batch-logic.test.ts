...
describe("Batch Command Logic Tests", () => {
    ...
    test("BatchCommand should execute multiple keyframe modifications at once", () => {
        const cmd1 = new ModifyKeyframeCommand(mockBridge, 0, 10, 
            { frame: 10, value: 200, interp: 0, easing: 0 }, 
            { frame: 15, value: 250, interp: 0, easing: 0 }
        );
        const cmd2 = new ModifyKeyframeCommand(mockBridge, 1, 10, 
        ...
        expect(mockState.animation.anim.parts[0].moves.some((m: any) => m[0] === 15 && m[1] === 250)).toBe(true);
        expect(mockState.animation.anim.parts[1].moves.some((m: any) => m[0] === 15 && m[1] === 450)).toBe(true);
        
        batch.undo();
        expect(mockState.animation.anim.parts[0].moves.some((m: any) => m[0] === 10 && m[1] === 200)).toBe(true);
        expect(mockState.animation.anim.parts[1].moves.some((m: any) => m[0] === 10 && m[1] === 400)).toBe(true);
    });
});
